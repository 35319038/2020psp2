...
l.6: ave_online関数の呼び出し
l.7: var_online関数の呼び出し
l.22:データファイル名を入力させ、fnameに格納
l.23:入力させた文字列の最後にエンターキーが入っているので、fnameの文字列の最後をヌル文字'\0'に置換
l.24:fnameを確認
l.26:fnameで指定したファイル委の先頭アドレスをファイルポインタfpに読み込む
l.27:もしfpがNULLだったときは、ファイルを開けなかったので即座にプログラム終了
...
- l.43のvar_onlineは，l.39のsquare_ave, l.41のaveを引数に入れてはダメですよ．
- 出力は，標本平均と標本分散だけではなく，母集団平均の推定値と母集団分散の推定値も出力してください．
 
l.40 呼ぶ順番を変更
l.40のsquare_aveにave_onlineを流用
l.53,l.54に母集団平均と母集団分散の推定値を出力

[comment #20200507]
- 課題1修正完了とします．以下は見ませんが，やっておいてください．
- レポートに入出力結果を書く場所を作ったので，実際の実行結果の表示をコピペしてください．
- 変数名をもう少し考えたほうがいいかと思います．1文字の変数名は自明でない限り，やめてください．特にbが不明．uはunbiased variance?
- 少しアドバイスですが，今のソースコードでは，`N`, `u`, `b` をグローバル変数にしています．変数のスコープの基本は「適用範囲を可能な限り狭く」です．
  - uとbはmain関数でしか使用していないので，グローバルでなく，main関数の中で宣言したほうがよい．
  - Nは関数でも使っているけれど，関数の引数として渡すのが望ましい．`ave=ave_online(val,ave,N)`などと．